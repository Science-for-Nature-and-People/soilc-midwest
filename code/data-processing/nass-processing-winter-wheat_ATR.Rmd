...
d.wheat.acres.total <- d.wheat.acres.total %>% 
  subset(short_desc == "WHEAT, WINTER - ACRES HARVESTED") %>% drop_na(Acres_total)
d.wheat.acres.total
```
...
d.wheat.15 <- d.wheat %>%
  ...
  ungroup(.) %>%
  dplyr::select(-n) #48692 obs of 8 variables
...
tic()
d_wheat.list_15.1 <- mclapply(X = d_wheat.list_15,FUN = mod, mc.cores = 28) 
toc()
# 161.737 sec elapsed
...
write_rds(d.wheat.15.1, file = "/home/aysha/Kane_data/winter_wheat_yield_2000-2022_w_irrigation_n_15.rds")
