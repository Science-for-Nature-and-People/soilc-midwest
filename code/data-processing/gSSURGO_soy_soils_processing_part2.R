...
crop.freq <- brick('/home/shared/Kane_data/CropScape/crop_frequency_soybeans_2008-2022.tif')


## ----soil------------------------------------------------------------------------------------------
all_counties_rds_list <- list.files("~/Kane_data/county_gssurgo_gdbs", full.names = T, pattern = ".rds")
...
soil.stats.gather <- function(i){
  ...
  temp.GEOID <- base::unique(i$GEOID)
  ...
  soil.stats.temp <- cbind(temp.GEOID,
                           ...
                           # ML: median does not work for large data size. disabled
...
gssurgo.soil.stats <- plyr::ldply(gssurgo.soil.stats) %>%
  ...
  # ML: removed medians from the final soil data
  rename_at(.vars = vars(2:13), .funs = function(x) paste("ssurgo", x, sep = "_")) %>%
...
saveRDS(gssurgo.soil.stats, file = "/home/aysha/Kane_data/gssurgo.soil.stats_soy_n_15.rds")
